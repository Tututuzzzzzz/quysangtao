...
import { ShieldCheck, UserCheck, Sparkles, Lock, Mail, Eye, EyeOff, ArrowLeft } from 'lucide-react';
...
export default function LoginModal({ onLoginSuccess }) {
  ...
  const handleQuickLogin = async (usr, pwd) => {
    ...
    try {
      ...
      setError('Đăng nhập nhanh thất bại. Vui lòng kiểm tra lại backend.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="bg-white/90 backdrop-blur-md rounded-3xl max-w-md w-full p-8 shadow-2xl border border-slate-200/80 space-y-6 relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between text-xs pt-1">
            <label className="flex items-center space-x-2 cursor-pointer select-none text-slate-600 font-medium">
              <input
                ...
                className="w-4 h-4 rounded border-slate-300 text-orange-500 focus:ring-orange-500/20 accent-orange-500"
              />
              <span>Ghi nhớ đăng nhập</span>
            </label>
            <button
              type="button"
              onClick={() => alert('Vui lòng liên hệ Ban Quản Trị LeadsGen (innovation@leadsgen.com) để hỗ trợ khôi phục mật khẩu.')}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-orange-500 to-amber-500 text-white text-xs font-extrabold shadow-md hover:from-orange-600 hover:to-amber-600 transition-all disabled:opacity-50"
          >
        ...
        <div className="pt-2 border-t border-slate-100 text-center">
          <Link
            to="/landing"
            className="inline-flex items-center gap-1 text-xs text-slate-500 hover:text-orange-600 font-semibold transition-colors"
          >
            <ArrowLeft className="w-3.5 h-3.5" />
            <span>Quay về Trang Giới Thiệu Dự Án</span>
          </Link>
        </div>
      </div>
    </div>
